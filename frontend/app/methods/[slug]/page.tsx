...
import { getMethodBySlug, type MethodDetail } from "@/lib/methods";
import MethodDetailComponent from "@/components/MethodDetail";
    ...
    <div className="space-y-4">
      ...
      {Array.from({ length: 3 }).map((_, i) => (
        <div key={i} className="ds-card p-5 flex flex-col gap-3 animate-pulse">
          ...
          <div className="h-4 bg-[#E5E5E0] rounded w-1/2" />
...
export default function MethodDetailPage() {
  ...
  const fetchMethod = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await getMethodBySlug(slug);
      setMethod(data);
    } catch (err: unknown) {
      if (err instanceof Error && err.message.includes("404")) {
        setError("Method not found");
      } else {
        setError("Failed to load method. Please try again later.");
      }
    } finally {
      setLoading(false);
    }
      ...
      <div className="min-h-screen bg-[#F8F7F2] text-[#111111]">
        <div className="w-full max-w-[1200px] mx-auto px-4 md:px-8 xl:px-12 py-8">
          ...
          <div className="flex flex-col items-center justify-center py-20 gap-3">
            ...
            <button onClick={fetchMethod} className="ds-button text-[12px]">
              Retry
            </button>
            <Link
              href="/methods"
  ...
  return <MethodDetailComponent method={method} />;
}
